      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 flex items-center gap-1.5 text-sm text-slate-500">
            ...
            {loading
              ? 'Syncing…'
              : `${products.length} personalizable · customer types name on store`}
          </p>
        </div>
        <button
          type="button"
              ...
              {!products.length && !loading && (
                <tr>
                  <td colSpan={5} className="px-4 py-10 text-center text-slate-400">
                    No Name it products yet. Add one above.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white shadow-xl">
            ...
            <form onSubmit={save} className="space-y-4 p-5">
              ...
              <p className="rounded-lg bg-mint-400/10 px-3 py-2 text-xs text-ink-700">
                Category fixed to <strong>Name it</strong>. Customer types name on store personalize page.
              </p>
              <div>
                ...
                {form.images.length < MAX_IMAGES && (
                  <div className="mt-3 space-y-2">
                    ...
                    <div className="flex gap-2">
                      <input className="flex-1 rounded-xl border px-3 py-2.5 text-sm" placeholder="Or paste image URL" value={urlInput} onChange={(e) => setUrlInput(e.target.value)} />
